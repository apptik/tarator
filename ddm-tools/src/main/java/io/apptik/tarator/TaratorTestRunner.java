 ...
 */
public class TaratorTestRunner implements TestRunner {
    ...
    public TaratorTestRunner(@Nullable File splitSelectExec,
    ...
    public boolean runTests(
            ...
            @NonNull ILogger logger) throws TestException, NoAuthorizedDeviceFoundException,
            InterruptedException {
        int threadPoolSize = maxThreads;
        if (mEnableSharding) {
            threadPoolSize = deviceList.size();
        }
        WaitableExecutor<Boolean> executor = WaitableExecutor.useNewFixedSizeThreadPool
                (threadPoolSize);

        int totalDevices = deviceList.size();
        int unauthorizedDevices = 0;
        final Map<DeviceConnector, ImmutableList<File>> availableDevices = new
                HashMap<DeviceConnector, ImmutableList<File>>();
                ...
                if (InstallUtils.checkDeviceApiLevel(
                    ...
                    if (!testData.isLibrary()) {
                        ...
                        if (testedApks.isEmpty()) {
                            logger.info("Skipping device '%1$s' for '%2$s:%3$s': No matching " +
                                    "output file",
        ...
        if (totalDevices == 0 || availableDevices.isEmpty()) {
            ...
            TestIdentifier fakeTest = new TestIdentifier(variantName,
                    totalDevices == 0 ? ": No devices connected." : ": No compatible devices " +
                            "connected.");
            ...
            if (unauthorizedDevices > 0) {
                ...
                // create a fake test output
                Map<String, String> emptyMetrics = Collections.emptyMap();
                TestIdentifier fakeTest = new TestIdentifier(variantName, ": found unauthorized " +
                        "devices.");
                fakeRunListener.testStarted(fakeTest);
                fakeRunListener.testFailed(fakeTest, String.format("Found %d unauthorized device" +
            ...
            if (mEnableSharding) {
                ...
                for (Map.Entry<DeviceConnector, ImmutableList<File>> runners : availableDevices
                        .entrySet()) {
                    CucumberTestData cucumberTestData = CucumberTestData.fromTestData(testData,
                            runners.getValue().get(0),
                            testApk);
                    CucTestCallable testCallable = new CucTestCallable(
                            (TaratorDevice) runners.getKey(), projectName, variantName,
                            cucumberTestData,
