 ...
 */
public class CucTestCallable implements Callable<Boolean> {
    ...
    @NonNull
    private final TaratorDevice device;
    @NonNull
    private final String flavorName;
    @NonNull
    private final CucumberTestData testData;
    @NonNull
    ...
    public CucTestCallable(
            @NonNull TaratorDevice device,
            @NonNull String projectName,
            @NonNull String flavorName,
            @NonNull CucumberTestData testData,
        ...
        this.testApk = testData.getTestApk();
        try {
            this.testedApks = testData.getTestedApks(null, null, null, null);
        ...
        if (device.getApiLevel() >= 23) {
            if (installOptions == null) {
                this.installOptions = ImmutableList.of("-g");
            } else if (!installOptions.contains("-g")) {
                if (installOptions instanceof ImmutableCollection) {
                    ArrayList<String> tmp = new ArrayList<>();
                    tmp.addAll(installOptions);
                    tmp.add("-g");
                    this.installOptions = ImmutableList.copyOf(tmp);
                } else {
                    installOptions.add("-g");
                    this.installOptions = ImmutableList.copyOf(installOptions);
                }
            } else {
                this.installOptions = ImmutableList.copyOf(installOptions);
            }
    ...
    public Boolean call() throws Exception {
        ...
        String coverageFile = "/data/data/" + testData.getTestedApplicationId() + "/" +
                FILE_COVERAGE_EC;

        String cucReportPath = "/data/data/" + testData.getTestedApplicationId() + "/cuc/";
        if (device.getApiLevel() >= 21) {
            cucReportPath = device.getExternalStoragePath()
                    + "/" + testData.getTestedApplicationId() + "/cuc/";
        }
        try {
            ...
            if (!testedApks.isEmpty()) {
                ...
                if (device.getApiLevel() >= 21 && testedApks.size() > 1) {
            ...
            if (device.getApiLevel() >= 23) {
                logger.verbose("granting EXTERNAL_STORAGE read/write");
                device.executeShellCommand("pm grant " + testData.getTestedApplicationId()
                                + " android.permission" + ".READ_EXTERNAL_STORAGE",
                        outputReceiver, timeoutInMs, TimeUnit.MILLISECONDS);
                device.executeShellCommand("pm grant " + testData.getTestedApplicationId()
            ...
            RemoteAndroidTestRunner runner = new RemoteAndroidTestRunner(
                    ...
                    device);
            logger.verbose("Cucumber reports path set to: " + cucReportPath);
            runner.addInstrumentationArg("plugin", "pretty" +
                    "--json:" + cucReportPath + "cuc.json" +
                    "--html:" + cucReportPath + "html");
            ...
            if (testRunResult.getNumTests() == 0) {
                ...
                TestIdentifier fakeTest = new TestIdentifier(device.getClass().getName(), "No " +
                        "tests found.");
                ...
                fakeRunListener.testFailed(
                        ...
                        "No tests found. This usually means that your test classes are"
                                + " not in the form that your test runner expects (e.g. don't " +
                                "inherit from"
            ...
            if (isInstalled) {
                ...
                if (testData instanceof CucumberTestData) {


                    logger.verbose("DeviceConnector '%s': fetching Cucumber data from %s",
                            deviceName, cucReportPath);

                    device.pull(new FileEntry[]{Nuts.obtainDirectoryFileEntry(cucReportPath)},
                            new File(resultsDir, deviceName + "-" + "cuc").getPath());

                    device.executeShellCommand("rm -fr " + cucReportPath,
                            outputReceiver,
    ...
    private void uninstall(@NonNull File apkFile, @Nullable String packageName,
        ...
        if (packageName != null) {
            ...
            logger.verbose("DeviceConnector '%s': unable to uninstall %s: unable to get package " +
                            "name",
                    deviceName, apkFile);
        }
    }
